...
fun createBaseItemQueueEntry(
	...
	entry.metadata =
		QueueEntryMetadata(
			...
			artworkUri =
				when {
					baseItem.imageTags?.containsKey(ImageType.PRIMARY) == true ->
						api.getImageUri(
							itemId = baseItem.id,
							tag = baseItem.imageTags!![ImageType.PRIMARY],
						)

					baseItem.albumPrimaryImageTag != null ->
						api.getImageUri(
