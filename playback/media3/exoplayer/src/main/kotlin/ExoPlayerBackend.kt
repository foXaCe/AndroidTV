...
class ExoPlayerBackend(
	...
	private val exoPlayer by lazy {
		...
		val player =
			if (exoPlayerOptions.enableLibAssRenderer) {
	...
	override fun supportsStream(stream: MediaStream): PlaySupportReport = exoPlayer.getPlaySupportReport(stream.toFormats())
	...
	override fun playItem(item: QueueEntry) {
		...
		var preparedItemIndex =
			(0 until exoPlayer.mediaItemCount).firstOrNull { index ->
			...
			if (trackIndex == index) {
				...
				exoPlayer.trackSelectionParameters =
					exoPlayer.trackSelectionParameters
						...
						.build()
				Timber.i("Selected audio track $index")
				return
			}
			trackIndex++
		}
		Timber.w("Audio track $index not found")
	}
			...
			if (trackIndex == index) {
				...
				exoPlayer.trackSelectionParameters =
					exoPlayer.trackSelectionParameters
						...
						.setOverrideForType(override)
						.build()
				Timber.i("Selected subtitle track $index")
				return
			}
			trackIndex++
		}
		Timber.w("Subtitle track $index not found")
	}
