...
class VideoSpeedControllerTests :
	FunSpec({
		afterTest {
		...
		test("VideoSpeedController speed is 1 by default") {
			...
			val slot = slot<Float>()
			every { mockController.playbackSpeed = capture(slot) } just Runs

			VideoSpeedController(mockController)

			verify { mockController.playbackSpeed = any() }
		...
		test("VideoSpeedController.resetSpeedToDefault() works correctly") {
			...
			val slot = slot<Float>()
			every { playbackController.playbackSpeed = capture(slot) } just Runs
			VideoSpeedController(playbackController)

			verify { playbackController.playbackSpeed = any() }
		...
		test("VideoSpeedController.currentSpeed always sets the requested speed when LiveTV is off") {
			...
			verify { mockController.playbackSpeed = 2.0f }
