...
class LibraryBrowseViewModelTests : FunSpec({
	...
	fun createVM() = LibraryBrowseViewModel(
	...
	test("retry clears error and starts loading when in genre mode") {
		...
		stateFlow.value = LibraryBrowseUiState(
			isGenreMode = true,
			genreName = "Action",
			libraryName = "Action",
		...
		vm.uiState.value.items.shouldBeEmpty()
	}
