...
class GenresGridViewModelTests : FunSpec({
	...
	fun createVM() = GenresGridViewModel(
	...
	@Suppress("UNCHECKED_CAST")
	fun setAllGenres(vm: GenresGridViewModel, genres: List<JellyfinGenreItem>) {
	...
	test("setSortOption updates sort and re-sorts genres") {
		...
		vm.uiState.value.genres.map { it.name } shouldBe listOf("Alpha", "Middle", "Zeta")
		vm.uiState.value.currentSort shouldBe GenreSortOption.NAME_ASC

		// Sort by name descending
		vm.setSortOption(GenreSortOption.NAME_DESC)
		vm.uiState.value.genres.map { it.name } shouldBe listOf("Zeta", "Middle", "Alpha")
	...
	test("retry clears error and sets loading") {
		...
		stateFlow.value = GenresGridUiState(
			error = UiError.Network(),
			isLoading = false,
		)

		// Verify error is set
		vm.uiState.value.error.shouldBeInstanceOf<UiError.Network>()
	...
	test("empty genres results in empty state with no error") {
		...
		vm.uiState.value.genres.shouldBeEmpty()
