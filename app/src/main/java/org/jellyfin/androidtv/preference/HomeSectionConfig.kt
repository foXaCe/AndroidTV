	...
	companion object {
		...
		fun defaults(): List<HomeSectionConfig> =
			listOf(
				...
				HomeSectionConfig(HomeSectionType.LIVE_TV, enabled = true, order = 1),
				HomeSectionConfig(HomeSectionType.LATEST_MEDIA, enabled = true, order = 2),
				HomeSectionConfig(HomeSectionType.RECENTLY_RELEASED, enabled = false, order = 3),
				...
				HomeSectionConfig(HomeSectionType.PLAYLISTS, enabled = false, order = 9),
			)
	}
}
