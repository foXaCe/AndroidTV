...
class JellyseerrHttpClient(
					...
					try {
						...
						if (element is JsonObject) {
							...
							if (fileContentsBase64 != null) {
								...
								val innerContentType =
									element["ContentType"]?.jsonPrimitive?.content
										?: "application/json"

								val newBody =
									decodedString.toResponseBody(
	...
	suspend fun getRequest(requestId: Int): Result<JellyseerrRequestDto> =
		runCatching {
	...
	suspend fun getTrending(
		...
		runCatching {
			val url =
				URLBuilder(apiUrl("discover/trending"))
					.apply {
	...
	suspend fun getTrendingMovies(
		...
		runCatching {
			val url =
				URLBuilder(apiUrl("discover/movies"))
					.apply {
						parameters.append("page", ((offset / limit) + 1).toString())
						parameters.append("language", "en")
					}.build()
	...
	suspend fun getTopMovies(
		...
		runCatching {
			val url =
				URLBuilder(apiUrl("discover/movies/top"))
					.apply {
	...
	suspend fun getTopTv(
		...
		runCatching {
			val url =
				URLBuilder(apiUrl("discover/tv/top"))
					.apply {
						parameters.append("limit", limit.toString())
						parameters.append("offset", offset.toString())
					}.build()
	...
	suspend fun getUpcomingMovies(
		...
		runCatching {
			val url =
				URLBuilder(apiUrl("discover/movies/upcoming"))
					.apply {
					}.build()
	...
	suspend fun getUpcomingTv(
		...
		runCatching {
			val url =
				URLBuilder(apiUrl("discover/tv/upcoming"))
					.apply {
					}.build()
	...
	suspend fun getMovieDetails(tmdbId: Int): Result<JellyseerrMovieDetailsDto> =
		runCatching {
			...
			response.body<JellyseerrMovieDetailsDto>()
		}.onFailure { error ->
	...
	suspend fun getTvDetails(tmdbId: Int): Result<JellyseerrTvDetailsDto> =
		runCatching {
			...
			response.body<JellyseerrTvDetailsDto>()
		}.onFailure { error ->
	...
	suspend fun loginJellyfin(
		...
		runCatching {
			...
			if (response.status.value == 500) {
				...
				val errorMessage =
					if (errorBody.contains("Something went wrong", ignoreCase = true)) {
	 ...
	 */
	suspend fun getStatus(): Result<JellyseerrStatusDto> =
		runCatching {
	...
	suspend fun getVegafoXStatus(): Result<VegafoXStatusResponse> =
		runCatching {
	...
	suspend fun vegafoxLogout(): Result<Unit> =
		runCatching {
	...
	suspend fun vegafoxValidate(): Result<VegafoXValidateResponse> =
		runCatching {
