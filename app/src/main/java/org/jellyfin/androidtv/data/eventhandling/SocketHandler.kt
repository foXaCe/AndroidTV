...
class SocketHandler(
	...
	private suspend fun onPlayStateMessage(message: PlaystateMessage) =
		withContext(Dispatchers.Main) {
					...
					when (message.data?.command) {
						...
						PlaystateCommand.SEEK ->
							playbackController?.seek(
								org.jellyfin.androidtv.data.syncplay.SyncPlayUtils
									.ticksToMs(message.data?.seekPositionTicks ?: 0),
							)
