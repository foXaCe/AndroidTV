...
class TimeSyncManager(
		...
		syncJob =
			scope.launch {
				while (isSyncing) {
					...
					val interval =
						if (_measurementCount < GREEDY_PING_COUNT) {
							GREEDY_INTERVAL_MS
						} else {
	...
	private suspend fun performSyncMeasurement() {
		try {
			...
			val t1 =
				response.content.requestReceptionTime.toEpochSecond(ZoneOffset.UTC) * 1000 +
					(response.content.requestReceptionTime.nano / 1_000_000)
			val t2 =
				response.content.responseTransmissionTime.toEpochSecond(ZoneOffset.UTC) * 1000 +
