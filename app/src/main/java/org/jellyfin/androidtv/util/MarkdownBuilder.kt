...
inline fun buildMarkdown(builderAction: MarkdownBuilder.() -> Unit): String = MarkdownBuilder().apply(builderAction).toString()

fun MarkdownBuilder.appendSection(
	name: String? = null,
	depth: Int = 3,
	content: MarkdownBuilder.() -> Unit,
) {
	if (name != null) {
...
fun MarkdownBuilder.appendItem(
	name: String,
	value: MarkdownBuilder.() -> Unit,
) {
	append("***$name***: ")
	value()
	appendLine("  ")
...
fun MarkdownBuilder.appendCodeBlock(
	language: String,
	code: String?,
) {
	if (last().category != CharCategory.LINE_SEPARATOR) appendLine()
...
fun MarkdownBuilder.appendDetails(
	summary: String? = null,
	content: MarkdownBuilder.() -> Unit,
) {
	appendLine()
