...
class SdkPlaybackHelper(
	...
	override fun getItemsToPlay(
		...
		getScope(context).launch {
			runCatching {
				...
				if (items.isEmpty() && !mainItem.mediaSources.isNullOrEmpty()) {
					listOf(mainItem)
				} else {
					items
				}
	...
	private suspend fun getItems(
		...
		withContext(Dispatchers.IO) {
			...
			fun List<BaseItemDto>.withServerIdPropagated(): List<BaseItemDto> = if (serverId != null) map { it.copy(serverId = serverId) } else this
			...
			when (mainItem.type) {
				...
				BaseItemKind.PROGRAM -> {
					...
					val channelWithProgramMetadata =
						channel.copy(
							serverId = serverId,
				...
				BaseItemKind.TV_CHANNEL -> {
					...
					if (currentProgram != null) {
						val channelWithCurrentProgramMetadata =
							channel.copy(
								serverId = serverId,
					...
					if (addIntros) {
						val intros =
							runCatching {
								itemApi.userLibraryApi
									.getIntros(mainItem.id)
									.content.items
							}.getOrNull()
								.orEmpty()
	...
	private suspend fun getParts(item: BaseItemDto): List<BaseItemDto> =
		buildList {
			add(item)
		...
		getScope(context).launch {
			...
			val pos =
				item.userData
					...
					?.ticks
					?.minus(resumeSubtractDuration) ?: Duration.ZERO
	...
	override fun retrieveAndPlay(
		...
		getScope(context).launch {
			...
			val pos =
				position?.ticks ?: items[0].userData?.playbackPositionTicks?.ticks?.minus(
	...
	private fun getScope(context: Context? = null) =
		when (context) {
