...
fun BaseItemDto.getProgramSubText(context: Context) =
	buildString {
...
fun BaseItemDto.getFullName(context: Context): String? =
	when (type) {
		BaseItemKind.EPISODE ->
			buildList {
				add(seriesName)
...
fun BaseItemDto.getSubName(context: Context): String? =
	when (type) {
		BaseItemKind.EPISODE ->
			when {
