...
class PlaybackManager(
	...
	private fun getApiClient(options: VideoOptions): ApiClient =
		if (options.serverId != null && apiClientFactory != null) {
	...
	fun changeVideoStream(
		...
		val sessionId = stream.playSessionId
		if (sessionId != null && stream.playMethod != PlayMethod.DIRECT_PLAY) {
			withContext(Dispatchers.IO) {
				apiClient.hlsSegmentApi.stopEncodingProcess(apiClient.deviceInfo.id, sessionId)
			}
		}
