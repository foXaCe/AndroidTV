...
class SimpleInfoRowView
	...
	constructor(
		...
		private fun setItemTextWithIcon(
			index: Int,
			text: String,
			...
			if (index < items.size) {
				...
				when (icon) {
					is RatingIconProvider.RatingIcon.LocalDrawable -> {
						...
						drawable?.setBounds(0, 0, 16.dp(context), 16.dp(context))
						textView.setCompoundDrawables(drawable, null, null, null)
						textView.compoundDrawablePadding = 4.dp(context)
					}
					is RatingIconProvider.RatingIcon.ServerUrl -> {
						val size = 16.dp(context)
