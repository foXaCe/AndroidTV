		...
		activeRows.forEach { rowType ->
			...
			item(key = key) {
				when (rowType) {
					...
					JellyseerrRowType.MOVIE_GENRES ->
						GenreRow(
							genres = movieGenres,
							mediaType = "movie",
							onGenreClick = onGenreClick,
						)
					JellyseerrRowType.SERIES_GENRES ->
						GenreRow(
							genres = tvGenres,
							mediaType = "tv",
							onGenreClick = onGenreClick,
						)
					JellyseerrRowType.STUDIOS ->
						LogoRow(
...
@Composable
private fun rowTitle(type: JellyseerrRowType): String =
	when (type) {
