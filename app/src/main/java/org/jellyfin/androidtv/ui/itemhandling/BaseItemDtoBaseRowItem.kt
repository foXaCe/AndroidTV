...
open class BaseItemDtoBaseRowItem
	@JvmOverloads
	constructor(
		item: BaseItemDto,
		...
		override fun getCardName(context: Context) =
			when {
		...
		override fun getImage(imageType: BaseRowImageType): JellyfinImage? {
			...
			val primaryImage =
				when {
					preferSeriesPoster && baseItem?.type == BaseItemKind.EPISODE ->
						baseItem.parentImages[ImageType.PRIMARY]
							?: baseItem.seriesPrimaryImage

					preferParentThumb && baseItem?.type == BaseItemKind.EPISODE ->
