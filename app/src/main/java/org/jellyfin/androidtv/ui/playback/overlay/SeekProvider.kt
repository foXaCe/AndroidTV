...
class SeekProvider(
	...
	fun getSeekPositions(): LongArray {
		if (!canSeek()) return LongArray(0)

		val dur = duration
		if (dur <= 0 || forwardTime <= 0) return LongArray(0)
		val size = ceil(dur.toDouble() / forwardTime.toDouble()).toInt() + 1
	...
	fun getThumbnail(index: Int, callback: ThumbnailCallback) {
		...
		val dur = duration
		if (dur <= 0 || forwardTime <= 0) return

		val currentTimeMs = (index * forwardTime).coerceIn(0, dur)
		val currentTile = currentTimeMs.floorDiv(trickPlayInfo.interval).toInt()
