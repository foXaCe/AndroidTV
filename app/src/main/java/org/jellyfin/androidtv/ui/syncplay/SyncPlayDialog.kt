...
fun SyncPlayDialog(
	...
	SettingsDialog(
		...
		SyncPlayContent(
			...
			availableGroups =
				availableGroups.map {
					GroupItem(it.groupId, it.groupName ?: context.getString(R.string.syncplay_unnamed_group), it.participants.size)
				},
					...
					coroutineScope.launch {
						...
						val groupName =
							if (userName !=
								...
								context.getString(R.string.syncplay_group_default_name, userName)
							} else {
								context.getString(R.string.syncplay_default_group_name)
							}
						...
						if (result.isFailure) {
							...
							val errorMessage =
								when {
									error is org.jellyfin.sdk.api.client.exception.InvalidStatusException &&
									...
									error is org.jellyfin.sdk.api.client.exception.InvalidStatusException &&
										...
										context.getString(
											R.string.syncplay_create_failed,
											error?.message ?: context.getString(R.string.lbl_unknown_error),
										)
								}
							...
							kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.Main) {
								android.widget.Toast
									.makeText(
										context,
										context.getString(
											R.string.syncplay_leave_failed,
											result.exceptionOrNull()?.message ?: context.getString(R.string.lbl_unknown_error),
										),
										android.widget.Toast.LENGTH_LONG,
									).show()
							}
		...
		item {
			ListSection(
				...
				headingContent = {
					Text(
						if (isInGroup) {
							stringResource(R.string.syncplay_in_group)
						} else {
							stringResource(R.string.syncplay_not_in_group)
						},
					)
				},
				captionContent = {
					Text(
						if (isInGroup) {
							stringResource(R.string.syncplay_group_info, groupName ?: "", participantCount)
						} else {
							stringResource(R.string.syncplay_description)
						},
					)
				},
			)
		...
		if (isInGroup) {
			...
			item {
				ListButton(
					leadingContent = {
						Icon(
							rememberVectorPainter(VegafoXIcons.SyncPlay),
			...
			item {
				ListButton(
					leadingContent = {
						Icon(
							rememberVectorPainter(VegafoXIcons.Logout),
							contentDescription = null,
							modifier = Modifier.size(24.dp),
						)
					},
			...
			item {
				ListButton(
					leadingContent = {
						Icon(
							rememberVectorPainter(VegafoXIcons.Add),
							contentDescription = null,
							modifier = Modifier.size(24.dp),
						)
					},
