...
class DestinationFragmentView
	...
	constructor(
		...
		private fun activateHistoryEntry(
			...
			if (fragment == null) {
				fragment =
					fragmentManager.fragmentFactory.instantiate(context.classLoader, entry.name.name).apply {
			...
			transaction.apply {
				...
				if (fragment.isDetached) {
					...
					replace(container.id, fragment, FRAGMENT_TAG_CONTENT)
				}
			}
