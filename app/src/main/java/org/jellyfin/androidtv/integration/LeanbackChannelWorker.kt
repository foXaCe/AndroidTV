...
class LeanbackChannelWorker(
			...
			.setPosterArtAspectRatio(
				...
				if (item.runTimeTicks?.ticks != null) {
					...
					val playbackPosition =
						item.userData?.playbackPositionTicks?.ticks
							?: Duration.ZERO
	...
	private fun updateWatchNext(nextUpItems: List<BaseItemDto>) {
		...
		context.contentResolver.bulkInsert(
			...
			programsToAdd
				.map { item -> getBaseItemAsWatchNextProgram(item).toContentValues() }
				.toTypedArray(),
		)
	}
	...
	private fun getCurrentWatchNext(): MutableList<WatchNextProgram> {
		...
		context.contentResolver
			.query(WatchNextPrograms.CONTENT_URI, WatchNextProgram.PROJECTION, null, null, null)
			.use { cursor ->
				...
				when {
					...
					(item.userData?.playbackPositionTicks ?: 0) > 0 -> {
						...
						setLastPlaybackPositionMillis(
							item.userData!!
								.playbackPositionTicks.ticks.inWholeMilliseconds
								.toInt(),
						)
